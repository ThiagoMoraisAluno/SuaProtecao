...
import { fetchClientsBySupervisor, fetchPlans } from "@/lib/api";
...
export function SupervisorDashboard() {
  ...
  useEffect(() => {
    if (!user) return;
    Promise.all([fetchClientsBySupervisor(user.id), fetchPlans()])
  ...
  const recentClients = useMemo(() => {
    return [...clients].sort((a, b) => new Date(b.joinedAt).getTime() - new Date(a.joinedAt).getTime()).slice(0, 5);
  }, [clients]);
    ...
    <SupervisorLayout>
      <div className="space-y-6 animate-fade-in">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <Link to="/supervisor/clients" className="btn-primary">
            <UserPlus size={18} />
            Novo Cliente
          </Link>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-slate-50 flex items-center justify-between">
            ...
            <Link to="/supervisor/clients" className="text-sm text-brand-600 hover:text-brand-700 font-medium">Ver todos →</Link>
          </div>
          {recentClients.length === 0 ? (
            <div className="p-16 text-center">
              <p className="text-slate-400 mb-4">Você ainda não cadastrou nenhum cliente</p>
              <Link to="/supervisor/clients" className="btn-primary justify-center">
                <UserPlus size={18} />
