...
export function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data: signUpData, error: signUpError } = await supabase.auth.signUp({
      ...
      options: { data: { full_name: form.name, username: form.name } },
    });
    ...
    await supabase.from("user_roles").insert({ id: userId, role: "client" });

    // 3. Update profile name
    await supabase.from("user_profiles").update({ username: form.name }).eq("id", userId);
    ...
    const { error: clientError } = await supabase.from("clients").insert({
      ...
      address_zip_code: form.zipCode,
    });

    if (clientError) {
      console.error("client insert error:", clientError);
      toast.error("Conta criada, mas houve um erro ao salvar seus dados. Contate o suporte.");
      setLoading(false);
      return;
    }
    ...
    const assets = Object.entries(form.selectedAppliances)
      ...
      .map(([name]) => ({
        client_id: userId,
        name,
        estimated_value: Number(form.applianceValues[name]) || 0,
      }));

    if (assets.length > 0) {
      await supabase.from("client_assets").insert(assets);
    }
          ...
          <div className="px-8 py-8">
                ...
                <div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="text" required value={form.name} onChange={(e) => set("name", e.target.value)} className="form-input" placeholder="Nome completo" />
                    </div>
                    <div>
                      <label className="form-label">CPF *</label>
                      <input type="text" required value={form.cpf} onChange={(e) => set("cpf", e.target.value)} className="form-input" placeholder="000.000.000-00" />
                    </div>
                    <div>
                      <label className="form-label">E-mail *</label>
                      <input type="email" required value={form.email} onChange={(e) => set("email", e.target.value)} className="form-input" placeholder="email@exemplo.com" />
                    </div>
                    <div>
                      <label className="form-label">Telefone</label>
                      <input type="tel" value={form.phone} onChange={(e) => set("phone", e.target.value)} className="form-input" placeholder="(00) 00000-0000" />
                    </div>
                    <div>
            ...
            {step === 2 && (
              <form onSubmit={handleStep2} className="space-y-6 animate-fade-in">
                <div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="text" required value={form.zipCode} onChange={(e) => set("zipCode", e.target.value)} className="form-input" placeholder="00000-000" />
                    </div>
                    <div>
                    ...
                    <div className="sm:col-span-2">
                      ...
                      <input type="text" required value={form.street} onChange={(e) => set("street", e.target.value)} className="form-input" placeholder="Nome da rua" />
                    </div>
                    <div>
                      <label className="form-label">Número *</label>
                      <input type="text" required value={form.number} onChange={(e) => set("number", e.target.value)} className="form-input" placeholder="123" />
                    </div>
                    <div>
                      <label className="form-label">Complemento</label>
                      <input type="text" value={form.complement} onChange={(e) => set("complement", e.target.value)} className="form-input" placeholder="Apto, sala, etc." />
                    </div>
                    <div>
                      <label className="form-label">Bairro *</label>
                      <input type="text" required value={form.neighborhood} onChange={(e) => set("neighborhood", e.target.value)} className="form-input" placeholder="Bairro" />
                    </div>
                    <div>
                      <label className="form-label">Cidade *</label>
                      <input type="text" required value={form.city} onChange={(e) => set("city", e.target.value)} className="form-input" placeholder="Cidade" />
                    </div>
                  </div>
                </div>
                <div className="flex gap-3">
                  ...
                  <button type="submit" className="flex-1 btn-primary justify-center py-3.5">
            ...
            {step === 3 && (
              <form onSubmit={handleStep3} className="space-y-6 animate-fade-in">
                      ...
                      return (
                        <button
                          ...
                          className={`p-4 rounded-xl border-2 text-left transition-all relative ${
                            isSelected ? "border-brand-500 bg-brand-50" : "border-slate-200 hover:border-slate-300 bg-white"
                          }`}
                        >
                          {plan.popular && (
                ...
                <div className="flex gap-3">
                  <button type="button" onClick={() => setStep(2)} className="btn-secondary px-5">
                    <ChevronLeft size={18} />
                  </button>
                  <button type="submit" className="flex-1 btn-primary justify-center py-3.5">
                    Próximo — Bens do Imóvel
                    <ArrowRight size={18} />
                  </button>
                </div>
              </form>
            ...
            {step === 4 && (
              <form onSubmit={handleSubmit} className="space-y-6 animate-fade-in">
                      ...
                      return (
                        <label
                          ...
                          className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${
                            checked ? "border-brand-300 bg-brand-50" : "border-slate-100 hover:border-slate-200"
                          }`}
                        >
                          <input
                            type="checkbox"
                            checked={checked}
                            onChange={(e) =>
                              set("selectedAppliances", { ...form.selectedAppliances, [appliance]: e.target.checked })
                            }
                          ...
                          {checked && (
                            <input
                              ...
                              onChange={(e) =>
                                set("applianceValues", { ...form.applianceValues, [appliance]: e.target.value })
                              }
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-slate-500 mt-0.5">
                            Limite de cobertura: {formatCurrency(selectedPlan.coverageLimit)}
                          </p>
                        </div>
                        <div className="text-right">
                          <p className={`text-xl font-bold font-display ${
                            totalAssetsValue > selectedPlan.coverageLimit ? "text-red-600" : "text-emerald-600"
                          }`}>
                ...
                <div className="flex gap-3">
                  <button type="button" onClick={() => setStep(3)} className="btn-secondary px-5">
                    <ChevronLeft size={18} />
                  </button>
