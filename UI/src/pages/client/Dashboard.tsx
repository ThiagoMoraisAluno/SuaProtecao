...
import { Shield, Wrench, AlertCircle, CheckCircle, PlusCircle, Phone, Home } from "lucide-react";
import { ClientLayout } from "@/components/layout/ClientLayout";
import { StatusBadge } from "@/components/features/StatusBadge";
import { useAuth } from "@/contexts/AuthContext";
import { fetchClientById, fetchPlans, fetchSupervisorById, fetchRequestsByClient } from "@/lib/api";
import { formatCurrency, getClientStatusConfig, getRequestStatusConfig, getServiceTypeLabel, getCoverageTypeLabel } from "@/lib/utils";
import { WHATSAPP_URL } from "@/constants";
import type { Client, Plan, Supervisor, Request } from "@/types";
...
export function ClientDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setRequests(r);
      if (c?.supervisorId) {
        const sup = await fetchSupervisorById(c.supervisorId);
        setSupervisor(sup);
      }
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        {isBlocked && (
          <div className="bg-red-50 border border-red-200 rounded-xl p-5 flex items-start gap-3">
            ...
            <div>
              ...
              <a href={WHATSAPP_URL} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 mt-3 bg-red-600 text-white text-sm font-semibold px-4 py-2 rounded-xl hover:bg-red-700 transition-colors">
        ...
        {plan && (
          <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
            <div className={`p-5 ${
              plan.type === "basic" ? "bg-slate-700" :
              plan.type === "intermediate" ? "bg-brand-600" : "bg-violet-700"
            } text-white`}>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <Link
            ...
            className={`flex items-center gap-4 p-5 bg-white rounded-xl border-2 shadow-sm transition-all ${
              isBlocked ? "border-slate-100 opacity-50 pointer-events-none" : "border-slate-100 hover:border-brand-200 hover:shadow-md group"
            }`}
          >
          ...
          <Link
            to="/client/new-request?type=coverage"
            className={`flex items-center gap-4 p-5 bg-white rounded-xl border-2 shadow-sm transition-all ${
              isBlocked ? "border-slate-100 opacity-50 pointer-events-none" : "border-slate-100 hover:border-purple-200 hover:shadow-md group"
            }`}
          >
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-4">
            ...
            <a href={WHATSAPP_URL} target="_blank" rel="noreferrer" className="flex items-center gap-4 p-5 bg-emerald-500 rounded-xl hover:bg-emerald-600 transition-colors">
        ...
        {recentRequests.length > 0 && (
          <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
            <div className="p-5 border-b border-slate-50 flex items-center justify-between">
              ...
              <Link to="/client/requests" className="text-xs text-brand-600 hover:text-brand-700 font-medium">Ver todos →</Link>
            </div>
