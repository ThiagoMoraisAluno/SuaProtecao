...
import { fetchSupervisors, fetchClients, createSupervisor } from "@/lib/api";
...
export function AdminSupervisors() {
  ...
  const [form, setForm] = useState({
    name: "", email: "", phone: "", commission: "10", password: "123456",
  });

  const loadData = async () => {
    const [s, c] = await Promise.all([fetchSupervisors(), fetchClients()]);
    setSupervisors(s);
    setClients(c);
    setLoading(false);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(err.message || "Erro ao cadastrar supervisor.");
    } finally {
      setSubmitting(false);
    }
  };
