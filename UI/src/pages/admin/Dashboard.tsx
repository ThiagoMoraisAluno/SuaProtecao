...
import { fetchClients, fetchSupervisors, fetchRequests, fetchPlans } from "@/lib/api";
import { formatCurrency, getRequestStatusConfig } from "@/lib/utils";
import type { Client, Supervisor, Request, Plan } from "@/types";
...
export function AdminDashboard() {
  ...
  useEffect(() => {
    Promise.all([fetchClients(), fetchSupervisors(), fetchRequests(), fetchPlans()])
      .then(([c, s, r, p]) => {
        setClients(c);
        setSupervisors(s);
        setRequests(r);
  ...
  }, [clients, supervisors, requests, plans]);
    ...
    <AdminLayout>
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <MetricCard title="Clientes Ativos" value={metrics.active} subtitle={clients.length > 0 ? `${Math.round((metrics.active / clients.length) * 100)}% do total` : "0%"} icon={<UserCheck size={22} />} colorClass="text-emerald-600" />
                    ...
                    <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all duration-500 ${
                          plan.type === "basic" ? "bg-slate-400" :
                          plan.type === "intermediate" ? "bg-brand-500" : "bg-violet-500"
                        }`}
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                      ...
                      <tr key={req.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="table-cell">
                          <span className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium ${
                            req.type === "service" ? "bg-blue-50 text-blue-700" : "bg-purple-50 text-purple-700"
                          }`}>
