...
export function AdminRequests() {
  ...
  const handleUpdate = async () => {
    ...
    setSaving(true);
    const updates: any = { status: newStatus, adminNotes };
          ...
          {[
            ...
            >
              ...
              <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${
                typeFilter === tab.value ? "bg-white/20 text-white" : "bg-slate-100 text-slate-600"
              }`}>{tab.count}</span>
            </button>
          ))}
        </div>
        ...
        <div className="space-y-3">
          {filtered.length === 0 ? (
            ...
            filtered.map((req) => {
              ...
              return (
                <div key={req.id} className="bg-white rounded-xl border border-slate-100 shadow-sm p-5 hover:shadow-md transition-shadow cursor-pointer" onClick={() => openDetail(req)}>
