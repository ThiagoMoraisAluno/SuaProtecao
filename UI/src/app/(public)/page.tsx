...
import {
  Shield, Wrench, Zap, CheckCircle, Phone, Star, ArrowRight,
  Home, Lock, Droplets, Building2,
} from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-white">
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-sm border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-3">
              <Link href="/login" className="text-sm font-semibold text-slate-700 hover:text-brand-600 transition-colors px-4 py-2 rounded-xl hover:bg-slate-50">
                Entrar
              </Link>
              <a href={WHATSAPP_URL} target="_blank" rel="noreferrer" className="btn-primary text-sm py-2 px-4">
                Contratar
              </a>
            </div>
          </div>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 lg:py-32">
          <div className="max-w-2xl">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <a href={WHATSAPP_URL} target="_blank" rel="noreferrer" className="btn-primary text-base py-3.5 px-8 justify-center">
                Contratar agora <ArrowRight className="w-5 h-5" />
              </a>
              <Link href="/login" className="inline-flex items-center justify-center gap-2 text-base font-semibold text-white border-2 border-white/30 hover:bg-white/10 px-8 py-3.5 rounded-xl transition-all duration-200">
                Já sou cliente
              </Link>
            </div>
      ...
      <section id="servicos" className="py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl lg:text-4xl font-bold text-slate-900 font-display mb-4">Tudo que seu lar precisa</h2>
            <p className="text-lg text-slate-500 max-w-2xl mx-auto">Profissionais qualificados para cada tipo de serviço, com atendimento ágil e garantido.</p>
          </div>
      ...
      <section id="planos" className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl lg:text-4xl font-bold text-slate-900 font-display mb-4">Escolha seu plano</h2>
            <p className="text-lg text-slate-500">Proteção completa para o seu perfil e orçamento.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            {PLANS.map((plan) => (
              <div key={plan.id} className={`relative rounded-2xl p-8 border-2 transition-all duration-200 hover:shadow-xl ${
                plan.popular ? "border-brand-500 bg-gradient-to-b from-brand-600 to-brand-800 text-white shadow-lg shadow-brand-500/20 scale-105" : "border-slate-100 bg-white hover:border-slate-200"
              }`}>
                {plan.popular && (
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2">
                    <span className="bg-gradient-to-r from-amber-400 to-orange-400 text-white text-xs font-bold px-4 py-1.5 rounded-full shadow-sm">⭐ Mais Popular</span>
                  </div>
                )}
                <div className="mb-6">
                  <h3 className={`text-xl font-bold font-display mb-2 ${plan.popular ? "text-white" : "text-slate-900"}`}>{plan.name}</h3>
                  <div className="flex items-end gap-1 mb-1">
                    <span className={`text-4xl font-bold font-display ${plan.popular ? "text-white" : "text-brand-600"}`}>{formatCurrency(plan.price)}</span>
                    <span className={`text-sm mb-1 ${plan.popular ? "text-white/70" : "text-slate-500"}`}>/mês</span>
                  </div>
                  <p className={`text-sm ${plan.popular ? "text-white/80" : "text-slate-500"}`}>Cobertura até {formatCurrency(plan.coverageLimit)}</p>
                </div>
                ...
                <a href={WHATSAPP_URL} target="_blank" rel="noreferrer"
                  className={`w-full flex items-center justify-center gap-2 py-3 rounded-xl font-semibold text-sm transition-all duration-200 ${
                    plan.popular ? "bg-white text-brand-700 hover:bg-brand-50" : "bg-brand-600 hover:bg-brand-700 text-white"
                  }`}>
      ...
      <section id="depoimentos" className="py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl lg:text-4xl font-bold text-slate-900 font-display mb-4">O que nossos clientes dizem</h2>
          </div>
      ...
      <section className="py-24 bg-gradient-to-br from-brand-700 to-brand-900">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl lg:text-4xl font-bold text-white font-display mb-6">Pronto para proteger seu lar?</h2>
          <p className="text-xl text-brand-200 mb-10">Comece com o plano que melhor se adapta às suas necessidades.</p>
