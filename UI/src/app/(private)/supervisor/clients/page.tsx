...
export default function SupervisorClientsPage() {
  ...
  const createMutation = useMutation({
    mutationFn: () => {
      ...
      const totalAssetsValue = assets.reduce((sum, a) => sum + a.estimatedValue, 0);
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["supervisor-clients"] });
      setShowForm(false);
      setForm(defaultForm);
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {showForm && (
        <div className="bg-white rounded-2xl border border-brand-200 shadow-sm animate-fade-in">
          ...
          <form onSubmit={(e) => { e.preventDefault(); createMutation.mutate(); }} className="p-6 space-y-8">
            ...
            <div>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div><label className="form-label">Nome Completo *</label>
                  <input type="text" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="form-input" placeholder="Nome completo" /></div>
                <div><label className="form-label">CPF *</label>
                  <input type="text" required value={form.cpf} onChange={(e) => setForm({ ...form, cpf: e.target.value })} className="form-input" placeholder="000.000.000-00" /></div>
                <div><label className="form-label">E-mail *</label>
                  <input type="email" required value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="form-input" placeholder="email@exemplo.com" /></div>
                <div><label className="form-label">Telefone</label>
                  <input type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="form-input" placeholder="(00) 00000-0000" /></div>
                <div><label className="form-label">Senha inicial *</label>
                  <input type="text" required value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} className="form-input" placeholder="Senha padrão" /></div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div><label className="form-label">CEP *</label>
                  <input type="text" required value={form.zipCode} onChange={(e) => setForm({ ...form, zipCode: e.target.value })} className="form-input" placeholder="00000-000" /></div>
                ...
                <div className="sm:col-span-2"><label className="form-label">Rua / Logradouro *</label>
                  <input type="text" required value={form.street} onChange={(e) => setForm({ ...form, street: e.target.value })} className="form-input" placeholder="Nome da rua" /></div>
                <div><label className="form-label">Número *</label>
                  <input type="text" required value={form.number} onChange={(e) => setForm({ ...form, number: e.target.value })} className="form-input" placeholder="123" /></div>
                <div><label className="form-label">Complemento</label>
                  <input type="text" value={form.complement} onChange={(e) => setForm({ ...form, complement: e.target.value })} className="form-input" placeholder="Apto, sala, etc." /></div>
                <div><label className="form-label">Bairro *</label>
                  <input type="text" required value={form.neighborhood} onChange={(e) => setForm({ ...form, neighborhood: e.target.value })} className="form-input" placeholder="Bairro" /></div>
                <div><label className="form-label">Cidade *</label>
                  <input type="text" required value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} className="form-input" placeholder="Cidade" /></div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid sm:grid-cols-3 gap-3">
                {plans.map((plan) => (
                  <button key={plan.id} type="button" onClick={() => setForm({ ...form, planId: plan.id })}
                    className={`p-4 rounded-xl border-2 text-left transition-all ${form.planId === plan.id ? "border-brand-500 bg-brand-50" : "border-slate-200 hover:border-slate-300"}`}>
            ...
            <div>
              ...
              <div className="grid sm:grid-cols-2 gap-2 mb-4">
                {APPLIANCES_LIST.map((appliance) => (
                  <label key={appliance} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${form.selectedAppliances[appliance] ? "border-brand-300 bg-brand-50" : "border-slate-100 hover:border-slate-200"}`}>
                    <input type="checkbox" checked={!!form.selectedAppliances[appliance]}
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="Buscar clientes..." value={search}
          onChange={(e) => setSearch(e.target.value)} className="form-input pl-10 max-w-sm" />
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={6} className="py-16 text-center text-slate-400">Nenhum cliente encontrado</td></tr>
              ) : (
                filtered.map((client) => {
