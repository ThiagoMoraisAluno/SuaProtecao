...
export default function AdminClientsPage() {
  ...
  const { data: clients = [], isLoading } = useQuery({ queryKey: ["clients"], queryFn: () => clientsService.findAll() });
  const { data: supervisors = [] } = useQuery({ queryKey: ["supervisors"], queryFn: () => supervisorsService.findAll() });
  const { data: plans = [] } = useQuery({ queryKey: ["plans"], queryFn: () => plansService.findAll() });

  const statusMutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: ClientStatus }) =>
      clientsService.updateStatus(id, status),
    onSuccess: () => {
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={8} className="text-center py-16 text-slate-400">Nenhum cliente encontrado</td></tr>
              ) : (
                filtered.map((client) => {
                  ...
                  return (
                    <tr key={client.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="table-cell">
                        <select
                          value={client.status}
                          onChange={(e) => statusMutation.mutate({ id: client.id, status: e.target.value as ClientStatus })}
