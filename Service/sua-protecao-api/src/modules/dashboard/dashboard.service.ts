...
export class DashboardService {
  ...
  async getAdminDashboard() {
    ...
    const openRequests = requests.filter((r) =>
      (
        [RequestStatus.pending, RequestStatus.in_progress, RequestStatus.analyzing] as RequestStatus[]
      ).includes(r.status),
    ).length;

    const pendingCoverage = requests.filter(
      (r) =>
        r.type === RequestType.coverage && r.status === RequestStatus.analyzing,
    ).length;
  ...
  async getClientDashboard(clientId: string) {
    ...
    const coverageUsedAmount = Number(
      coverageUsed._sum.approvedAmount ?? 0,
    );
